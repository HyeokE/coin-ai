...
import { DecisionLog, PositionEvalLog, TradeLog } from '../models/log.model';
...
export class SupabaseLogger {
  ...
  public async logPositionEval(log: PositionEvalLog): Promise<void> {
    if (!this.enabled || !this.client) return;

    try {
      const { error } = await this.client.from('position_eval_logs').insert(log);
      if (error) throw error;
    } catch (err) {
      console.error('Failed to log position eval:', err);
    }
  }
