...
import { getSymbolRiskPolicy, getSymbolConfig } from '../config/config';
...
function simpleStrategy(debug = false): DecisionProvider {
  ...
  return (candles, signal, position) => {
    ...
    } else if (signal.direction === 'DOWN' && rsi > 30 && currentPrice < sma20) {
      side = TradeSide.SHORT;
      confidence = Math.min(90, 50 + (rsi - 30));
    }
    ...
    const stopLoss = side === TradeSide.LONG ? currentPrice * 0.98 : currentPrice * 1.02;
    const targetPrice = side === TradeSide.LONG ? currentPrice * 1.04 : currentPrice * 0.96;
    ...
    return {
      ...
      stopLoss,
      targetPrice,
      reasoning: `${signal.type} + RSI(${rsi.toFixed(1)}) + Price ${side === TradeSide.LONG ? '>' : '<'} SMA20`,
    };
  };
}
...
async function fetchCandles(symbol: string, type: CandleType, count: number): Promise<Candle[]> {
  ...
  while (remaining > 0) {
    const fetchCount = Math.min(remaining, maxPerRequest);

    if (type === 'days') {
      const upbitCandles = await marketService.getDaysCandles({
        market: symbol,
        count: fetchCount,
        to,
      });
      if (upbitCandles.length === 0) break;
      ...
      to = upbitCandles[upbitCandles.length - 1].candle_date_time_utc;
    } else {
      const upbitCandles = await marketService.getMinutesCandles({
        market: symbol,
        unit: 5,
        count: fetchCount,
        to,
    ...
    if (remaining > 0) {
      process.stdout.write(`   Fetched ${allCandles.length}/${count}...\r`);
...
async function main(): Promise<void> {
  ...
  const candleCount = parseInt(process.env.CANDLE_COUNT || '200', 10);
