...
export class VolatilityTrigger {
  ...
  public analyze(candles: Candle[]): VolatilitySignal | null {
    ...
    const atrSignal = this.checkATRSpike(candles);
    if (atrSignal) return atrSignal;

    const surgeSignal = this.checkPriceSurge(candles);
    if (surgeSignal) return surgeSignal;

    const volumeSignal = this.checkVolumeSpike(candles);
    if (volumeSignal) return volumeSignal;

    return null;
  }

  private checkATRSpike(candles: Candle[]): VolatilitySignal | null {
    ...
    const threshold = avgATR * this.config.volatilityThresholds.atrMultiplier;
  ...
  private checkPriceSurge(candles: Candle[]): VolatilitySignal | null {
    const recent = candles.slice(-5);
    const startPrice = recent[0].open;
    const endPrice = recent[recent.length - 1].close;
    const changePercent = ((endPrice - startPrice) / startPrice) * 100;
    const threshold = this.config.volatilityThresholds.priceSurgePercentValue;
  ...
  private checkVolumeSpike(candles: Candle[]): VolatilitySignal | null {
